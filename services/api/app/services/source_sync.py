...
def refresh_ramp_sources(db: Session, ramp: Ramp, use_fixture: bool = False) -> dict:
    ...
    for period in periods:
        ...
        wf = WeatherForecast(
            ...
            valid_until=datetime.fromisoformat(normalized['valid_until'].replace('Z', '+00:00')) if normalized['valid_until'] else None,
    ...
    for feature in alert_payload.get('features', []):
        ...
        alert = Alert(
            source='nws',
            source_alert_id=props.get('id') or props.get('@id') or props.get('event') + '-' + now.isoformat(),
            ramp_id=ramp.id,
            ...
            effective_at=datetime.fromisoformat(props['effective'].replace('Z', '+00:00')) if props.get('effective') else None,
            expires_at=datetime.fromisoformat(props['expires'].replace('Z', '+00:00')) if props.get('expires') else None,
            ends_at=datetime.fromisoformat(props['ends'].replace('Z', '+00:00')) if props.get('ends') else None,
            raw_payload=props,
        )
        db.add(alert)

    station = db.scalar(select(Station).where(Station.provider == 'coops', Station.provider_station_id == '8726520', Station.station_type == 'tide'))
    if not station:
        station = Station(
            provider='coops',
    ...
    db.query(TidePrediction).filter(TidePrediction.station_id == station.id).delete(synchronize_session=False)
    for row in coops_payload.get('predictions', [])[:400]:
        try:
            predicted_at = datetime.fromisoformat(row['t'].replace(' ', 'T') + '-04:00').astimezone(timezone.utc)
        except Exception:
            continue
        pred = TidePrediction(
    ...
    buoy_station = db.scalar(select(Station).where(Station.provider == 'ndbc', Station.provider_station_id == '41002', Station.station_type == 'buoy'))
    if not buoy_station:
    ...
    ndbc = NdbcClient()
    txt = load_ndbc_fixture('ndbc_41002.txt') if use_fixture else load_ndbc_fixture('ndbc_41002.txt')
    spec = load_ndbc_fixture('ndbc_41002.spec') if use_fixture else load_ndbc_fixture('ndbc_41002.spec')
    ...
    if parsed_txt:
        existing_obs = db.scalar(
            select(Observation)
            .where(Observation.station_id == buoy_station.id, Observation.observed_at == parsed_txt['observed_at'], Observation.source == 'ndbc')
        )
        if existing_obs:
        ...
        obs = Observation(
            ...
            dominant_period_sec=parsed_txt.get('dominant_period_sec') or parsed_spec.get('dominant_period_sec'),
            average_period_sec=parsed_txt.get('average_period_sec') or parsed_spec.get('average_period_sec'),
            mean_wave_direction_deg=parsed_txt.get('mean_wave_direction_deg') or parsed_spec.get('mean_wave_direction_deg'),
            ...
            raw_payload={'txt': parsed_txt.get('raw_payload'), 'spec': parsed_spec.get('raw_payload')},
        )
        db.add(obs)
