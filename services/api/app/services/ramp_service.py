...
from app.models.entities import Alert, LaunchScore, Observation, Ramp, RampReport, SavedRamp, TidePrediction, User, WeatherForecast
...
def get_ramp_conditions(db: Session, ramp_id: str) -> dict:
    ...
    alerts = list(db.scalars(select(Alert).where(or_(Alert.ramp_id == ramp_id, Alert.ramp_id.is_(None)))))
...
def save_ramp_for_user(db: Session, user: User, ramp_id: str) -> SavedRamp:
    ...
    existing = db.scalar(select(SavedRamp).where(SavedRamp.user_id == user.id, SavedRamp.ramp_id == ramp_id))
    if existing:
        return existing

    current_count = db.scalar(select(func.count(SavedRamp.id)).where(SavedRamp.user_id == user.id)) or 0
...
def remove_saved_ramp(db: Session, user: User, ramp_id: str) -> None:
    saved = db.scalar(select(SavedRamp).where(SavedRamp.user_id == user.id, SavedRamp.ramp_id == ramp_id))
    if not saved:
        return
    db.delete(saved)
    db.commit()
...
def get_saved_ramps(db: Session, user: User) -> list[SavedRamp]:
    return list(db.scalars(select(SavedRamp).where(SavedRamp.user_id == user.id).order_by(SavedRamp.created_at.desc())))


def create_ramp_report(db: Session, user_id: str | None, ramp_id: str, report_type: str, message: str | None, photo_url: str | None) -> RampReport:
    get_ramp_or_404(db, ramp_id)
    report = RampReport(user_id=user_id, ramp_id=ramp_id, report_type=report_type, message=message, photo_url=photo_url)
    db.add(report)
    db.commit()
    db.refresh(report)
    return report
...
def get_launch_windows(db: Session, ramp_id: str, profile_id: str | None = None, days: int = 7) -> list[LaunchScore]:
