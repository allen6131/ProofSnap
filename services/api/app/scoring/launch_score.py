...
from app.models.entities import Alert, LaunchScore, Observation, Ramp, TidePrediction, UserProfile, WeatherForecast
...
def _thresholds_for_profile(profile: UserProfile | None) -> Thresholds:
    ...
    return Thresholds(
        ...
        min_tide_height_ft_mllw=float(profile.min_tide_height_ft_mllw) if profile.min_tide_height_ft_mllw is not None else None,
...
def _reason(severity: str, code: str, message: str, source: str, value: float | None = None, threshold: float | None = None) -> dict:
...
def _evaluate_color(
    ...
    if not forecast and not observation:
        reasons.append(_reason('info', 'missing_critical_data', 'Critical forecast and observation data missing.', 'RampReady'))
    ...
    for alert in alerts:
        ...
        if 'fog advisory' in event or 'coastal flood advisory' in event:
            reasons.append(_reason('caution', 'moderate_alert', f'Active advisory: {alert.event}', 'NWS'))

    wind = float(forecast.wind_speed_kt) if forecast and forecast.wind_speed_kt is not None else None
    gust = float(forecast.wind_gust_kt) if forecast and forecast.wind_gust_kt is not None else None
    wave = float(forecast.wave_height_ft) if forecast and forecast.wave_height_ft is not None else None
    thunder = float(forecast.thunderstorm_probability_pct) if forecast and forecast.thunderstorm_probability_pct is not None else None

    obs_gust = float(observation.wind_gust_kt) if observation and observation.wind_gust_kt is not None else None
    obs_wave = float(observation.wave_height_ft) if observation and observation.wave_height_ft is not None else None

    if wind is not None and wind > thresholds.max_wind_kt:
        reasons.append(_reason('danger', 'wind_high', f'Wind {wind:.1f} kt exceeds limit.', 'NWS', wind, thresholds.max_wind_kt))
        return 'red', 20.0, reasons, False, wave is None, False
    if gust is not None and gust > thresholds.max_gust_kt:
        reasons.append(_reason('danger', 'gust_high', f'Gust {gust:.1f} kt exceeds limit.', 'NWS', gust, thresholds.max_gust_kt))
        return 'red', 20.0, reasons, False, wave is None, False
    if wave is not None and wave > thresholds.max_wave_height_ft:
        reasons.append(_reason('danger', 'wave_high', f'Wave {wave:.1f} ft exceeds limit.', 'NWS', wave, thresholds.max_wave_height_ft))
    ...
    if ramp.min_recommended_tide_ft_mllw is not None:
        min_tide = max(min_tide or float(ramp.min_recommended_tide_ft_mllw), float(ramp.min_recommended_tide_ft_mllw))
    if min_tide is not None and tide and tide.tide_height_ft_mllw is not None:
        tide_h = float(tide.tide_height_ft_mllw)
        if tide_h < min_tide:
            reasons.append(_reason('danger', 'low_tide', f'Tide {tide_h:.1f} ft below minimum.', 'NOAA CO-OPS', tide_h, min_tide))
            return 'red', 20.0, reasons, False, wave is None, False
        if tide_h < min_tide + 0.3:
            reasons.append(_reason('caution', 'tide_near_min', f'Tide near minimum: {tide_h:.1f} ft.', 'NOAA CO-OPS', tide_h, min_tide))

    if thunder is not None and thunder >= 50:
        reasons.append(_reason('danger', 'thunderstorm_risk', 'Thunderstorm probability is high.', 'NWS', thunder, 50))
        return 'red', 20.0, reasons, False, wave is None, False
    ...
    if obs_gust is not None and gust is not None and gust > 0 and obs_gust > gust * 1.25:
        ...
        if obs_gust > thresholds.max_gust_kt:
            reasons.append(_reason('danger', 'observed_gust_high', f'Observed gust {obs_gust:.1f} kt exceeds your limit.', 'NDBC/CO-OPS', obs_gust, thresholds.max_gust_kt))
            return 'red', 20.0, reasons, mismatch, wave is None, False
        reasons.append(_reason('caution', 'observed_gust_mismatch', 'Observed gust is worse than forecast.', 'NDBC/CO-OPS', obs_gust, gust))

    if obs_wave is not None and wave is not None and wave > 0 and obs_wave > wave * 1.3:
        mismatch = True
        if obs_wave > thresholds.max_wave_height_ft:
            reasons.append(_reason('danger', 'observed_wave_high', f'Observed wave {obs_wave:.1f} ft exceeds your limit.', 'NDBC', obs_wave, thresholds.max_wave_height_ft))
            return 'red', 20.0, reasons, mismatch, False, False
        reasons.append(_reason('caution', 'observed_wave_mismatch', 'Observed waves are worse than forecast.', 'NDBC', obs_wave, wave))
    ...
    if near_limit(wind, thresholds.max_wind_kt):
        reasons.append(_reason('caution', 'wind_near_limit', f'Wind is near your limit ({wind:.1f} kt).', 'NWS', wind, thresholds.max_wind_kt))
        caution = True
    if near_limit(gust, thresholds.max_gust_kt):
        reasons.append(_reason('caution', 'gust_near_limit', f'Gust is near your limit ({gust:.1f} kt).', 'NWS', gust, thresholds.max_gust_kt))
        caution = True
    if near_limit(wave, thresholds.max_wave_height_ft):
        reasons.append(_reason('caution', 'wave_near_limit', f'Wave is near your limit ({wave:.1f} ft).', 'NWS', wave, thresholds.max_wave_height_ft))
        caution = True
    ...
    if caution or mismatch:
        if not reasons:
            reasons.append(_reason('caution', 'borderline_conditions', 'Some factors are close to limits.', 'RampReady'))
        return 'yellow', 60.0, reasons, mismatch, wave is None, False

    reasons.append(_reason('good', 'conditions_ok', 'Conditions are below your configured limits.', 'RampReady'))
    if wind is not None:
        reasons.append(_reason('good', 'wind_ok', f'Wind {wind:.1f} kt is below your limit.', 'NWS', wind, thresholds.max_wind_kt))
    return 'green', 85.0, reasons, mismatch, wave is None, False


def build_launch_windows(db: Session, ramp: Ramp, user_profile_id: str | None, days: int = 7) -> list[LaunchScore]:
    ...
    observations = list(db.scalars(select(Observation).order_by(Observation.observed_at.desc()).limit(8)))
    tides = list(db.scalars(select(TidePrediction).order_by(TidePrediction.predicted_at.asc()).limit(days * 8)))
    alerts = list(db.scalars(select(Alert).where((Alert.ramp_id == ramp.id) | (Alert.ramp_id.is_(None)))))
    ...
    for i in range(days * 8):
        ...
        forecast = next((f for f in forecasts if f.valid_time <= w_end and (f.valid_until is None or f.valid_until >= w_start)), None)
        tide = next((t for t in tides if t.predicted_at <= w_end and t.predicted_at >= w_start - timedelta(hours=3)), None)
        ...
        if color == 'green' and confidence < 70:
            color = 'yellow'
            reasons.append(_reason('caution', 'confidence_medium', 'Data confidence is below high threshold.', 'RampReady', confidence, 70))
        if confidence < 30 and color != 'red':
            color = 'gray'
            reasons.append(_reason('info', 'low_confidence', 'Not enough fresh data for a responsible score.', 'RampReady', confidence, 30))
