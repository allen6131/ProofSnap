...
def get_ramp(ramp_id: str, db: Session = Depends(get_db)) -> dict:
    ...
    return {
        ...
        'min_recommended_tide_ft_mllw': float(ramp.min_recommended_tide_ft_mllw) if ramp.min_recommended_tide_ft_mllw else None,
...
def report_ramp_issue(ramp_id: str, payload: RampIssueRequest, user=Depends(get_optional_user), db: Session = Depends(get_db)) -> dict:
    report = create_ramp_report(db, user.id if user else None, ramp_id, payload.report_type, payload.message, payload.photo_url)
    return {'id': report.id, 'status': report.status}


@router.post('/me/saved-ramps/{ramp_id}')
def add_saved_ramp(ramp_id: str, user=Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
...
def delete_saved_ramp(ramp_id: str, user=Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
