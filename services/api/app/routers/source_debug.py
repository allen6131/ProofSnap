...
def source_status(db: Session = Depends(get_db)) -> dict:
    ...
    latest_forecast = db.scalar(
        select(WeatherForecast).order_by(WeatherForecast.created_at.desc()).limit(1)
    )
    ...
    return {
        "weather": {
            ...
            "updated_at": latest_forecast.created_at if latest_forecast else None,
            "freshness_minutes": age(latest_forecast.created_at) if latest_forecast else None,
        },
        "alerts": {
            "source": "NWS active alerts",
            "active_count": db.query(Alert).count(),
            "checked_at": now,
        },
        "tide": {
            "source": "NOAA CO-OPS",
            "updated_at": latest_tide.predicted_at if latest_tide else None,
            "freshness_minutes": age(latest_tide.predicted_at) if latest_tide else None,
        },
        "buoy": {
            "source": "NOAA National Data Buoy Center",
            "updated_at": latest_obs.observed_at if latest_obs else None,
            "freshness_minutes": age(latest_obs.observed_at) if latest_obs else None,
        },
    }
...
def ramp_source_summary(ramp_id: str, db: Session = Depends(get_db)) -> dict:
    ...
    return {
        ...
        "forecast": {
            "source": latest_forecast.source if latest_forecast else None,
            "updated_at": latest_forecast.created_at if latest_forecast else None,
        },
        "alerts": {"count": len(alerts), "events": [a.event for a in alerts]},
        "observation": {
            "source": latest_obs.source if latest_obs else None,
