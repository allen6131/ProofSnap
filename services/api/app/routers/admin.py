...
def admin_update_ramp(ramp_id: str, payload: RampUpdateRequest, db: Session = Depends(get_db)) -> dict:
    ramp = get_ramp_or_404(db, ramp_id)
...
def admin_get_station_links(ramp_id: str, db: Session = Depends(get_db)) -> list[dict]:
    ...
    return [{k: v for k, v in l.__dict__.items() if not k.startswith('_')} for l in links]


@router.post('/ramps/{ramp_id}/station-links')
def admin_create_station_link(ramp_id: str, payload: StationLinkCreateRequest, db: Session = Depends(get_db)) -> dict:
...
def admin_update_station_link(link_id: str, payload: StationLinkUpdateRequest, db: Session = Depends(get_db)) -> dict:
...
def admin_import_osm(payload: dict, db: Session = Depends(get_db)) -> dict:
    run = JobRun(job_name='import_osm_ramps', status='success', details={'note': 'Dev-only placeholder', 'bbox': payload.get('bbox')})
    db.add(run)
    db.commit()
    return {'ok': True, 'note': 'OSM import is dev-only for MVP. Public Overpass should not be production backend.'}


@router.post('/recompute-links')
def admin_recompute_links(db: Session = Depends(get_db)) -> dict:
    run = JobRun(job_name='link_ramps_to_stations', status='success', details={'note': 'Spatial linking placeholder for MVP vertical slice'})
    db.add(run)
    db.commit()
    return {'ok': True}
...
def admin_recompute_scores(db: Session = Depends(get_db)) -> dict:
    run = JobRun(job_name='recompute_launch_scores', status='success', details={'note': 'Scores recomputed lazily on request'})
    db.add(run)
    db.commit()
    return {'ok': True}
