...
def put_profile(payload: UserProfileRequest, user=Depends(get_current_user), db: Session = Depends(get_db)) -> UserProfileResponse:
    profile = db.scalar(select(UserProfile).where(UserProfile.user_id == user.id))
...
def list_saved_ramps(user=Depends(get_current_user), db: Session = Depends(get_db)) -> list[dict]:
    ...
    for row in saved:
        ...
        latest_score = db.scalar(
            select(LaunchScore).where(LaunchScore.ramp_id == row.ramp_id, LaunchScore.user_profile_id == user.profile.id).order_by(LaunchScore.starts_at.asc()).limit(1)
        )
        results.append(
            {
                'id': row.id,
...
def dashboard(user=Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    ...
    for saved_ramp in saved:
        ...
        score = db.scalar(
            select(LaunchScore)
            .where(LaunchScore.ramp_id == saved_ramp.ramp_id, LaunchScore.user_profile_id == user.profile.id)
        ...
        rows.append(
            {
                ...
                'active_alerts': score.source_summary.get('alerts', {}).get('active_count', 0) if score else 0,
            }
        )
