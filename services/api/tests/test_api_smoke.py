...
def test_register_login_and_ramp_flow(client, db_session):
    register = client.post('/auth/register', json={'email': 'demo@example.com', 'password': 'password123'})
    assert register.status_code == 200

    login = client.post('/auth/login', json={'email': 'demo@example.com', 'password': 'password123'})
